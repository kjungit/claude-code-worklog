...
Every push and pull request against `main`/`dev` also runs `.github/workflows/test.yml` (the unit test suite) and `.github/workflows/validate.yml` (`claude plugin validate --strict` against both manifests).
